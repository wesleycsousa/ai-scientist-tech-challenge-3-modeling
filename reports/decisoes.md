...
**Como ler este arquivo:** é um registro cronológico, escrito enquanto o projeto acontecia. Os títulos "Dia 1", "Dia 2" etc. seguem o plano de trabalho em sete etapas que usei pra organizar o projeto, e cada "Achado N" é um ponto numerado pra facilitar referência cruzada: os notebooks, o `reports/insights.md` e o README citam os achados por esse número. Este arquivo funciona como anexo técnico do README: lá está o resumo das decisões; aqui fica o detalhe e a ordem em que cada uma foi tomada, inclusive as que foram revistas depois (sempre marcadas no próprio achado como "Atualização", "superada" ou "revertida"). Todas as pendências abertas ao longo do texto foram fechadas em achados posteriores.
...
Este bloco reúne tudo que apurei e discuti até agora na EDA (`notebooks/eda/01_eda.ipynb`), pra servir de referência única antes de fechar as decisões finais de modelagem. Separei o que já era decisão fechada do que ainda estava em aberto naquele momento; todos os pontos em aberto foram fechados nos achados seguintes (a mudança para o grão de escola, mais abaixo, tornou vários deles sem efeito).
...
**Decisão:** métrica principal de avaliação no Dia 4 = AUC-ROC + F1 da classe "não alfabetizado" (mais custoso, num contexto de política pública, deixar de identificar quem está em risco), com acurácia reportada só como complemento. **Atualização:** no grão de escola, a comparação entre modelos acabou usando acurácia em validação cruzada (o alvo ficou quase balanceado) e a AUC entrou como métrica complementar na validação final (Achados 38 e 43).
...
Segundo, se a não-participação tem um padrão territorial: tem, e curiosamente **não repete o mapa geográfico da alfabetização**. Por região, a taxa de não-participação é Sul 18,12% > Norte 17,98% > Sudeste 12,55% > Centro-Oeste 11,75% > Nordeste 9,16% — note que o Sul (boa alfabetização) tem a maior não-participação, e o Nordeste (alfabetização mediana-baixa) tem a menor. Por rede, o padrão é mais intuitivo: Estadual 14,42% > Municipal 13,13% > Privada 4,00% (rede privada com participação bem mais garantida). Registro isso como um achado à parte: não resolve o tratamento dessas colunas no modelo de alfabetização, e acabou não sendo explorado depois da mudança para o grão de escola, mas fica como uma extensão possível (mapa de não-participação como indicador complementar de risco educacional).

**Decisão sobre o tratamento:** na época deixei em aberto entre excluir as colunas, restringir o escopo do modelo a alunos presentes/com prova preenchida, ou manter como parte legítima da definição oficial do indicador. **Ficou sem efeito com a mudança de granularidade para escola** (seção "Decisão de arquitetura", abaixo): com o alvo vindo do IDEB, essas duas colunas não entram nem como feature nem no cálculo do alvo. Também percebi que o pequeno grupo de alunos sem dado de infraestrutura (3.209 alunos, 0,1% da base) tem taxa de alfabetização de exatamente 0% (Seção 12) — cheguei a especular que pudesse ser o mesmo fenômeno do grupo de leakage, mas a escala não bate (3.209 é ~160x menor que os 513.338 afetados pela regra de ausência) — na melhor hipótese seria um subconjunto pequeno, não o mesmo grupo. Essa checagem direta ficou superada pelo mesmo motivo: a base de alunos deixou de ser usada.

### Achado 7 — `taxa_alfabetizacao_ano_anterior` e `variacao_pp`: não é o mesmo tipo de problema que `presenca`/`preenchimento_caderno`

Essas duas colunas vêm da tabela Gold `evolucao_temporal` (granularidade `(nivel_geografico, local_id, ano)`, filtrada em `rede="5"` — Pública Estadual+Municipal, conforme a documentação da Fase 2), trazidas via `ler_evolucao_temporal_municipio()`. São dados de **município**, não de aluno — todo aluno do mesmo município no mesmo ano recebe o mesmo valor. Existem só pra 2024 (2023 fica nulo, porque não há um "ano anterior" dentro da janela de dados que temos).
...
**Decisão:** usar `GroupKFold`/`GroupShuffleSplit` por `id_municipio` na validação do Dia 3, não um split ou `StratifiedKFold` comum. **Atualização:** decisão revertida no grão de escola (Achado 34): `train_test_split` estratificado simples, porque cada linha passou a ser uma escola só.

### Achado 9 — Ausência de dado de infraestrutura é rara e parece estar ligada ao grupo de ausentes

Apenas 0,1% dos alunos (3.209) não têm dado de infraestrutura do município — e esse grupo tem taxa de alfabetização de exatamente 0,00% (Seção 12), o que bate com o padrão do Achado 6 e sugere que pode ser o mesmo subconjunto de alunos ausentes/prova não preenchida. A estratégia de imputação para esse grupo ficou superada pela mudança para o grão de escola; no pipeline final a imputação é mediana para numéricas e categoria "desconhecido" para categóricas (Achado 33).
...
**Impacto no que já foi feito:** a EDA da Opção A (`notebooks/eda/01_eda.ipynb`, Seções 1-14) não é descartada — ela é exatamente a evidência que fundamenta essa decisão, e continua sendo commitada como parte do Dia 2. As pendências que dependiam da granularidade aluno/município (uso de `taxa_alfabetizacao_ano_anterior`, arquitetura de "score de município") ficaram sem efeito com a nova granularidade (ver Achados 13 e 14).

**Questões que essa decisão abriu** (todas fechadas depois: alvo no Achado 14, interseção de `id_escola` no Achado 11, seleção de features nos Achados 20 a 33, leakage no Achado 15):
...
**Decisão:** usá-la ou não como feature direta do modelo é uma questão sensível: incluir sem tratamento pode fazer o modelo aprender um proxy racial em vez de um fator realmente acionável por política pública (como infraestrutura ou corpo docente). Deixei para resolver na seleção final de features, fora da EDA, e fechei no Achado 23: exclusão total das colunas de raça/cor.

## Dia 3 — Fechando a seleção de features das 455 colunas

Esta seção fecha a seleção de features, aplicando um funil decisivo de redução de dimensionalidade em `notebooks/eda/08_selecao_features_escola.ipynb` sobre a base montada por `obter_base_escola_modelagem()`. Documento aqui as decisões que fechei, deixando explícito o critério de cada uma. Já rodei o notebook do início ao fim contra a base real (348 colunas, 63.537 escolas) — os números abaixo, e o funil completo na Seção 13 do notebook, já são reais, não estimativa.
...
**Atualização — resolvido no Achado 26:** cheguei a concluir aqui que a tabela `dicionario` do `br_inep_censo_escolar` nunca tinha sido ingerida pro S3 do projeto, e deixei uma consulta especulativa (via `basedosdados.read_sql`) pronta pra tentar mais tarde. Fiz essa ingestão depois de fechar esta etapa, e o proxy manual por prefixo `tipo_` descrito acima foi substituído por uma classificação exata baseada no dicionário — ver Achado 26.
...
### Achado 27 (decisão fechada) — Seções 6.1 e 7 do notebook 07 rodadas com output real
...
Com isso, não sobra nenhuma seção do notebook 07 sem execução registrada.
...
Importante não confundir isso com a decisão sobre a chave de agrupamento do split treino/validação: são duas perguntas diferentes. `sigla_uf` virar feature não implica nada sobre qual coluna seria usada como grupo no split; essa segunda decisão foi fechada no Achado 34.
...
**Dois bugs reais que só apareceram testando contra a base de verdade (antes de fechar esta etapa isso pronto):**
...
**Ponto verificado:** o `load_data.py` original tem uma nota de que minhas credenciais só têm permissão de leitura nos buckets da Fase 2 (`silver/alunos/`, `gold/...`). Rodei `build_base_escola.py` de verdade e a escrita em `silver_modelo/` funcionou sem erro de permissão — minha credencial cobre esse prefixo novo dentro do bucket do projeto.
...
**Gap que continua real, não é bug:** `tipo_localizacao_diferenciada` ainda mostra `"codigo_nao_documentado_0"` (58.154 escolas, a maioria) e `"codigo_nao_documentado_8"` (263 escolas) — inspecionando o dicionário de verdade, a linha `id_tabela="escola"` desta coluna só documenta os códigos 1, 2, 3 e 4; o código 0 só aparece documentado pra `id_tabela="turma"` (como `"Não se aplica"`). Decidi não reaproveitar a tradução da tabela `turma` para esta coluna (seria assumir que "Não se aplica" vale para o código 0 também no grão escola, sem confirmação na fonte). Ela ficou como `codigo_nao_documentado_0` na base publicada e, na revisão final (Achado 33), `tipo_localizacao_diferenciada` não entrou na lista de features, então o gap não afeta o modelo.
...
### Achado 37 — Três decisões fecham a virada do baseline pra otimização: descarte do Naive Bayes, Random Forest como quinto candidato, e formalização do treino/validação/teste

A comparação dos quatro baselines (Achado 35) deixou claro que dava pra fechar três decisões antes de começar a etapa de otimização, no `notebooks/modelagem/10_otimizacao_modelo.ipynb`:

1. **Descarto o Naive Bayes daqui pra frente.** Foi o modelo mais fraco do lote (0,655 de acurácia) e, mais importante que a acurácia sozinha, o `classification_report` mostrou um desbalanceamento grande entre as classes (recall de 0,93 pra "Sim" contra só 0,32 pra "Não", Achado 35), sinal de que a assunção de independência entre features do Naive Bayes não segura bem nesta base, onde várias colunas de infraestrutura são correlacionadas entre si. Não faz sentido gastar tempo de otimização num modelo que já perde de largada e ainda erra de um jeito estruturalmente desequilibrado.
2. **Adiciono Random Forest como quinto candidato.** No teste informal que motivou o Achado 36 (só pra olhar `feature_importances_`, fora do escopo oficial do baseline), a Random Forest com hiperparâmetros default já saiu com acurácia de 0,744, próxima do SVM (0,749) e muito mais rápida de treinar (segundos, não minutos). Como ensemble de árvores, ainda dá pra explicar via `feature_importances_`, ficando como meio-termo entre a Árvore de Decisão (visual, mas fraca sozinha) e o SVM (forte, mas caixa-preta).
3. **Formalizo a separação treino/validação/teste.** No baseline, comparei os quatro modelos direto no conjunto de teste (Achado 35) - aceitável pra uma primeira comparação entre famílias de modelo, mas não é correto repetir esse tipo de olhada pra escolher hiperparâmetro: cada vez que decido algo olhando o resultado no teste, gasto um pouco da capacidade dele de medir generalização de verdade. A partir do notebook de otimização, o treino (os mesmos 80% de sempre) é usado só pra buscar hiperparâmetro via validação cruzada, e o teste (os mesmos 20%, fixados por `random_state=42`) vira a validação final, só aberta uma vez, depois de escolhido o modelo final.

### Achado 38 — Otimização de hiperparâmetro dos três candidatos, e nomenclatura treino/teste/validação formalizada

No `notebooks/modelagem/10_otimizacao_modelo.ipynb`, rodei busca de hiperparâmetro pros modelos que sobraram do baseline (Naive Bayes descartado, Random Forest incluído - Achado 37), usando `GridSearchCV`/`RandomizedSearchCV` com o `Pipeline` completo (pré-processador + classificador) passado inteiro pra dentro da busca. Isso evita que a estatística de imputação/escala vaze entre os folds da validação cruzada, diferente de pré-processar uma vez só antes de dividir em folds.

Resultado (acurácia média em validação cruzada, dentro do treino):

| Modelo | Melhor combinação | Acurácia média (CV) | Tempo de busca |
|---|---|---|---|
| Random Forest | `n_estimators=200, max_depth=None, min_samples_leaf=1` | 0,7479 | 660,4s |
| SVM | `C≈15,70, gamma≈0,00625` (kernel RBF) | 0,7465 | 1050,0s (cerca de 17,5min) |
| Regressão Logística | `C=10, penalty=l1 (liblinear)` | 0,7379 | 62,5s |
| Árvore de Decisão | `max_depth=None, min_samples_leaf=50, criterion=gini` | 0,7294 | 162,1s |

(Tempos da execução final registrada no notebook; numa primeira rodada, com a máquina menos carregada, a mesma busca tinha levado 88,9s, 21,2s e 70,7s, respectivamente. A ordem relativa entre os modelos não muda.)

Random Forest fica na frente, seguido de perto pelo SVM (que não seguiu pra otimização - ver Achado 39 logo abaixo). Regressão Logística melhora com a regularização certa, mas fica atrás. Árvore de Decisão, mesmo otimizada, é a mais fraca dos três candidatos que seguem no processo.

Pro SVM, usei `StratifiedKFold` de só 3 folds (contra 5 dos outros três) e `RandomizedSearchCV` com `n_iter=6` em vez de grade completa - um único fit de SVM com kernel RBF numa fração do treino do tamanho de um fold levou cerca de 46 segundos num teste isolado antes de montar o notebook, tornando uma grade completa proibitiva em tempo. `C`/`gamma` sorteados de distribuição log-uniforme, já que os dois têm efeito multiplicativo, não aditivo, na prática.

**Nomenclatura ajustada - treino / teste / validação.** Até aqui eu vinha chamando o recorte de 20% que fica de fora da busca inteira de "teste" ou "cofre fechado" (Achado 34). A partir desta etapa, padronizo pra três nomes: treino é o que entra na busca de hiperparâmetro; teste, nesse contexto, é o holdout de cada fold da validação cruzada, embutido dentro do próprio treino, não um conjunto separado; validação é o recorte de 20% que fica fora da busca inteira. O que antes eu chamava de `X_teste`/`y_teste` virou `X_validacao`/`y_validacao` no notebook 10 - continua sendo exatamente o mesmo recorte de escolas, com o mesmo `random_state=42` do notebook 09, só mudou o nome.

**Mudança de plano sobre quando abrir a validação.** No Achado 34, eu tinha decidido abrir esse recorte só uma vez, depois de escolher o modelo final. Decidi abrir mais cedo, comparando treino (via validação cruzada) contra validação nos três candidatos que seguem no processo ao mesmo tempo (SVM fora, Achado 39), não só no escolhido, porque quero enxergar se algum deles generaliza pior do que a acurácia média da validação cruzada sozinha sugere, antes de fechar a escolha. É um desvio do plano original de abrir a validação só uma vez, só pro modelo final - registro aqui pra deixar claro que foi decisão consciente, não esquecimento da própria regra que eu tinha fechado.

Métricas usadas nessa comparação: Acurácia, Precisão, Recall e F1-score (`src/evaluation/metricas.py::comparar_metricas_completas`), mais matriz de confusão de cada modelo na validação (`src/visualization/graficos.py::plot_matrizes_confusao`).

**Atualização:** o comparativo treino x validação está nas Seções 7 e 8 do notebook 10: diferença abaixo de 0,3 ponto percentual entre a acurácia média em validação cruzada e a acurácia na validação, pros três otimizados, sem sinal de overfitting. A leitura que embasa a escolha final (Árvore: 0,7294 em CV contra 0,7315 na validação, AUC 0,791) está no Achado 43.

### Achado 39 — SVM sem busca de hiperparâmetro, mas mantido na comparação como candidato básico

Decidi não otimizar hiperparâmetro do SVM. O Achado 38 já mostra por quê: mesmo reduzindo a busca (`RandomizedSearchCV`, poucas iterações, 3 folds em vez de 5), o SVM levou 1050 segundos (quase 17,5 minutos) pra chegar a 0,7465 de acurácia média em CV, pior que o Random Forest, que chegou a 0,7479 na própria busca em grade (660 segundos na execução final registrada no notebook, ainda assim menos que a busca do SVM). O custo da busca do SVM não compensa o ganho.

Isso não tira o SVM da comparação: ele continua entrando, só que na versão básica (hiperparâmetros default do `SVC`, os mesmos usados no `09_baseline_classificacao.ipynb`), sem passar pela etapa de busca. Nessa versão básica, o SVM ficou em 0,7456 de acurácia média em validação cruzada (400 segundos só pra validação cruzada). Um único fit de SVM default é rápido o bastante - o custo alto vinha das várias combinações de `C`/`gamma` testadas na busca, não do fit em si. Sigo a etapa de otimização de hiperparâmetro com os outros três candidatos (Árvore de Decisão, Random Forest, Regressão Logística), e comparo os quatro juntos (três otimizados + SVM básico) nas Seções 7 e 8 do notebook.

### Achado 40 — Confirmação explícita: a validação nunca entrou em `.fit()` durante a otimização

Ponto que vale registrar sem ambiguidade, porque é fácil de errar sem perceber. Em toda a etapa de otimização (Achados 37-39), a base de validação (`X_validacao`/`y_validacao`) nunca participou de nenhum `.fit()`, nem do `ColumnTransformer`, nem de nenhum classificador, nem de nenhuma busca de hiperparâmetro. Ela só aparece do lado direito de um `.predict()`, sempre depois que o modelo correspondente já estava treinado inteiramente com o treino. Isso vale tanto pro `GridSearchCV`/`RandomizedSearchCV` (que dividem o treino em folds internamente, sem tocar na validação) quanto pro re-fit final de cada `best_estimator_` (feito automaticamente pelo scikit-learn sobre o treino inteiro). O único uso da validação foi simular a chegada de dado novo em produção: perguntar como um modelo já pronto se sai numa escola que ele nunca viu, não usar essa resposta pra ajustar nada do modelo em si.

### Achado 41 — Quanto cada modelo melhorou sobre o baseline

Com a busca de hiperparâmetro fechada (Achado 38) e o SVM básico rodado (Achado 39), dá pra comparar contra os números originais do baseline (Achado 35):

| Modelo | Baseline (defaults) | Otimizado / básico | Ganho |
|---|---|---|---|
| Árvore de Decisão | 0,675 (teste, `max_depth=4`) | 0,7294 (CV, `max_depth=None, min_samples_leaf=50`) | +5,44 p.p. |
| Random Forest | 0,744 (teste informal, defaults) | 0,7479 (CV, `n_estimators=200, min_samples_leaf=1`) | +0,39 p.p. |
| Regressão Logística | 0,737 (teste, defaults) | 0,7378 (CV, `C=10, penalty=l1`) | +0,08 p.p. |
| SVM | 0,749 (teste, defaults) | sem busca (Achado 39) | não se aplica |

A Árvore de Decisão foi de longe quem mais se beneficiou de ajuste de hiperparâmetro - o `max_depth=4` do baseline (escolhido só por questão de visualização, não por desempenho) estava deixando bastante acurácia na mesa. Random Forest e Regressão Logística já estavam perto do próprio teto com os defaults, então o ganho da busca foi pequeno, sinal de que esses dois modelos são menos sensíveis a hiperparâmetro nesta base do que a Árvore. Vale o cuidado de leitura: o número "baseline" veio de uma única leitura no teste (Achado 35), enquanto o "otimizado" é média de validação cruzada no treino - não é uma comparação no mesmo pé exato, mas a diferença de metodologia é pequena o suficiente pra não mudar a leitura geral.

### Achado 42 — Por que o split real é 80/20 + validação cruzada, não um 60/20/20 literal

Vale esclarecer um ponto que ficou confuso entre o meu planejamento inicial (que citava a referência de curso de um split treino/val/teste 60/20/20) e o que de fato implementei. Nunca cheguei a criar uma terceira partição física fixa de dados. O único corte físico que existe, do início ao fim, é o `train_test_split` de 80/20 do Achado 34: 80% treino, 20% validação, sempre com o mesmo `random_state=42`.

**No baseline (notebook 09), não havia busca de hiperparâmetro nenhuma** - cada modelo foi treinado uma vez, com os hiperparâmetros default, direto nos 80% de treino. Como não existia nenhuma escolha sendo feita a partir do resultado, avaliar esses modelos uma única vez nos 20% restantes não tinha risco de vazamento: esse recorte só serviu pra simular a chegada de dado novo (uma escola que o modelo nunca viu), sem influenciar nenhuma decisão de configuração do modelo.

**Na otimização (notebook 10), a busca de hiperparâmetro muda esse cenário.** Se eu usasse os mesmos 20% pra escolher qual combinação de hiperparâmetro é "melhor", estaria vazando informação da validação pro processo de decisão, mesmo sem nenhum `.fit()` direto nela - a escolha do hiperparâmetro já seria, de forma indireta, ajustada a esse recorte. Pra evitar isso, a busca inteira (`GridSearchCV`/`RandomizedSearchCV`) acontece só dentro dos 80% de treino: o `StratifiedKFold` divide esse treino em fatias, e cada fatia serve de vez em quando de holdout interno (o que passei a chamar de "teste", Achado 38) enquanto o resto do treino ajusta o modelo - tudo isso é reamostragem dentro dos mesmos 80%, nunca toca nos 20% de validação. Só depois de escolhido o hiperparâmetro vencedor de cada modelo (com base só na média da validação cruzada, dentro do treino) é que abro a validação, uma única vez, via `.predict()` (Achado 40) - o mesmo papel de "simular produção" que ela já tinha no baseline, só que agora protegido contra um risco de vazamento que o baseline não tinha (porque o baseline não tinha busca nenhuma pra vazar).

**Resumindo a diferença entre os dois notebooks:** no baseline, a validação valida "isso funciona em dado novo?" sem risco de vazamento, porque nenhuma decisão foi tomada em cima dela. Na otimização, a validação cruzada dentro do treino resolve o problema de "como escolher hiperparâmetro sem vazar", e a validação de 20% continua cumprindo o mesmo papel de simular produção, agora depois de todo o processo de ajuste já ter sido fechado só com o treino. O 60/20/20 citado como referência de curso descreve uma forma alternativa de resolver o mesmo problema (reservar fisicamente um terceiro pedaço só pra tunar hiperparâmetro); usar validação cruzada dentro do treino, como fiz aqui, é uma alternativa igualmente válida e mais comum na prática, porque aproveita o treino inteiro tanto pra ajustar quanto pra validar cada combinação de hiperparâmetro, sem precisar reservar um terceiro pedaço fixo de dado que ficaria menor e mais sujeito a ruído por sorte de amostragem.

### Achado 43 (decisão final) — Modelo escolhido: Árvore de Decisão

Entre os quatro candidatos otimizados (Achado 38: Random Forest 0,7479, SVM básico 0,7456, Regressão Logística 0,7379, Árvore de Decisão 0,7294), fecho a escolha do modelo final em **Árvore de Decisão**, por dois motivos, nessa ordem de peso:

1. **Interpretabilidade.** É o único candidato que dá pra abrir e ler a regra de decisão diretamente (Seção 10 do notebook 10: `plot_tree` com profundidade limitada só no desenho, não no modelo treinado), sem depender de nenhuma técnica de interpretabilidade pós-hoc tipo SHAP. Isso importa porque o problema é de política pública (sinalizar escola em risco de não bater a meta do IDEB): quem for usar o modelo precisa conseguir explicar por que uma escola específica foi sinalizada, não só apresentar um número de acurácia. Random Forest e SVM (kernel RBF) são caixa-preta na prática; Regressão Logística dá coeficiente, mas não a mesma leitura direta de regra condicional que uma árvore dá.
2. **Diferença de performance pequena.** A distância entre a Árvore (0,7294) e o topo (Random Forest, 0,7479) é de 1,85 ponto percentual de acurácia média em validação cruzada — pequena o suficiente pra não parecer decisiva sozinha, especialmente peso contra o ganho de interpretabilidade.

**Testei também se uma busca de hiperparâmetro bem mais pesada melhora esse quadro**, num notebook separado (`notebooks/modelagem/11_otimizacao_avancada_arvore.ipynb`): 400 combinações via `RandomizedSearchCV` (contra 64 combinações em grade fixa no notebook 10), escala log (`loguniform`) no `ccp_alpha`, os dois critérios (`gini` e `entropy`), e um critério de seleção que, entre as combinações com melhor acurácia de validação cruzada, escolhe a de menor gap treino-CV (pra não cair numa combinação que decorou o treino). O resultado é honesto e um pouco contraintuitivo: a busca pesada achou uma combinação com acurácia de CV levemente maior (0,7304 contra 0,7294), mas essa árvore generaliza pior na validação (0,7224 de acurácia e AUC de 0,785) do que a árvore mais simples do notebook 10 (0,7315 de acurácia e AUC de 0,791 na validação, com gap treino-validação favorável). Ou seja, a busca maior não trouxe ganho real fora da amostra de treino, mesmo com o cuidado explícito de penalizar overfitting no critério de escolha.

**Configuração final da Árvore de Decisão:** `criterion="gini"`, `max_depth=None` (sem limite), `min_samples_leaf=50`, a mesma da busca leve do notebook 10 — não a configuração mais complexa (`max_depth=15`, `min_samples_split=52`, `ccp_alpha≈0,00029`) encontrada na busca pesada do notebook 11, que custou 1360 segundos e não generalizou melhor. Fica registrado como teste feito e descartado, não como etapa desperdiçada: reforça que a diferença de acurácia entre os candidatos é pequena e sensível a ruído de busca, o que pesa a favor de decidir com base em interpretabilidade em vez de perseguir o último ponto percentual de acurácia.

### Achado 44 — Hipóteses analíticas formuladas na etapa de EDA (grão escola)

Registrando aqui, de forma consolidada, as hipóteses analíticas levantadas ainda na etapa de exploração dos dados no grão escola — essa formulação é etapa própria do processo (entender o problema antes de modelar), mesmo que a confirmação de várias delas só tenha vindo depois, já em achados numerados anteriores. As cinco hipóteses:

1. **Infraestrutura da escola está associada ao IDEB.** Confirmada (Achado 27, Seção 5 do notebook 07): várias colunas de infraestrutura aparecem com correlação alta contra o alvo. Ressalva importante levantada junto (Achado 29): correlação não implica causalidade direta — uma coluna como "tem TV" provavelmente funciona como proxy de um pacote mais amplo de investimento/recurso, não como causa isolada do desempenho.
2. **`sigla_uf` e `rede` carregam sinal de governança, não só geografia/administração genérica.** Confirmada (Achados 27, 28 e 36): `sigla_uf` é a categórica com maior variação de `% acima da meta` entre todas as testadas (80,2% em SP contra 18-24% em MA/PA/RN/SE/AP), e aparece entre as features mais importantes de uma Random Forest exploratória. A leitura correta não é só "região geográfica importa", é que a rede estadual de cada UF administra a escola (orçamento, contratação, manutenção) de um jeito que se reflete no desempenho.
3. **Tamanho da escola (matrícula) influencia o desempenho.** Testada via a escolha de `quantidade_matricula_fundamental_anos_iniciais` como feature de porte (Achado 22) — a coluna sobrevive ao funil de seleção de features (Achado 33), então participa do modelo, mas não recebeu um teste de hipótese isolado e formal (tipo comparação de IDEB médio por faixa de porte); fica como uma limitação a registrar, não uma lacuna escondida.
4. **Localização (urbana/rural) e outras categóricas de contexto têm relação com o alvo.** Confirmada (Achado 27, Seção 7): todas as categóricas testadas, incluindo `tipo_localizacao`, mostram diferença real de `% acima da meta` entre categorias.
5. **Composição racial da escola correlaciona com o alvo, mas não deve virar feature.** Confirmada a correlação (Achado 19) e fechada a decisão de exclusão total das colunas de raça/cor do conjunto de features (Achado 23) — o motivo não é falta de poder preditivo (o oposto: a correlação existe), é o risco do modelo aprender e reproduzir estatisticamente uma desigualdade estrutural em vez de fatores acionáveis por política pública. Fica registrado como limitação explícita: mesmo excluída a variável direta, o sinal racial pode vazar indiretamente via variáveis correlacionadas (ex.: localização/região).

Das cinco, a única que não teve uma checagem quantitativa própria e isolada é a de número 3 (tamanho da escola) — as outras quatro já tinham evidência real reunida em achados anteriores, o que essa seção faz é consolidar essa evidência explicitamente como resposta a hipóteses formuladas na etapa de EDA, não gerar teste novo.
